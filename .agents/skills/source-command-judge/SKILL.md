...
Use this skill when the user asks to run the migrated source command `judge`.
...
`Read tests/fixtures/NNN-label.bundle.json`. Produce a fresh structured verdict from the bundle **alone**, applying the same strategy and rules as `/analyze` (`.Codex/commands/analyze.md` — the 3-pillar checklist, `docs/strategy/`). Emit these six fields:
