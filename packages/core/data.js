...
export function _processPineLinesPayload(raw, { verbose, max_lines } = {}) {
  ...
  const studies = raw.map(s => {
    ...
    for (const item of s.items) {
      ...
      if (verbose) allLines.push({ id: item.id, y1, y2, x1: v.x1, x2: v.x2, style: v.st, width: v.w, color: v.ci });
      if (!verbose && y1 != null && v.y1 === v.y2 && !seen[y1]) { hLevels.push(y1); seen[y1] = true; }
    }
    if (verbose && max_lines && allLines.length > max_lines) {
      allLines.sort((a, b) => (a.x1 ?? 0) - (b.x1 ?? 0));
      allLines = allLines.slice(-max_lines);
    }
    if (verbose) {
      return { name: s.name, total_lines: s.count, showing: allLines.length, all_lines: allLines };
    }
...
export async function getPineLines({ study_filter, verbose, max_lines } = {}) {
  const filter = study_filter || '';
  const raw = await evaluate(buildGraphicsJS('dwglines', 'lines', filter));
  return _processPineLinesPayload(raw, { verbose, max_lines });
}

export async function getPineLabels({ study_filter, max_labels, verbose, text_excludes } = {}) {
  const filter = study_filter || '';
  const raw = await evaluate(buildGraphicsJS('dwglabels', 'labels', filter));
  if (!raw || raw.length === 0) return { success: true, study_count: 0, studies: [] };

  const limit = max_labels || 50;
  const excludes = Array.isArray(text_excludes) ? text_excludes.filter(p => typeof p === 'string' && p.length > 0) : [];
  const studies = raw.map(s => {
    let labels = s.items.map(item => {
      const v = item.raw;
      const text = v.t || '';
      const price = v.y != null ? Math.round(v.y * 100) / 100 : null;
      if (verbose) return { id: item.id, text, price, x: v.x, textColor: v.tci };
      return { text, price };
    }).filter(l => l.text || l.price != null);
    const total_unfiltered = labels.length;
    let dropped_by_text_excludes = 0;
    if (excludes.length > 0) {
      const before = labels.length;
      labels = labels.filter(l => !excludes.some(p => (l.text || '').startsWith(p)));
      dropped_by_text_excludes = before - labels.length;
    }
    if (labels.length > limit) labels = labels.slice(-limit);
    const out = { name: s.name, total_labels: s.count, showing: labels.length, labels };
    if (excludes.length > 0) out.dropped_by_text_excludes = dropped_by_text_excludes;
    return out;
  });
  return { success: true, study_count: studies.length, studies };
}
...
export async function getPineBoxes({ study_filter, verbose } = {}) {
  ...
  const studies = raw.map(s => {
    ...
    for (const item of s.items) {
      ...
      else if (high != null && low != null) { const key = high + ':' + low; if (!seen[key]) { zones.push({ high, low }); seen[key] = true; } }
    }
    if (verbose) return { name: s.name, total_boxes: s.count, all_boxes: allBoxes };
