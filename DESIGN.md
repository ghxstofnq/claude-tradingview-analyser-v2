...
- **Segmented pills.** `.cs-segpill` (transparent + 1px `--border` inactive → `--cs-onfill` + `--cs-onborder` active) for filters/toggles; `.cs-provpill` (borderless, same on-fill) for provider/view tabs; `.cs-sympill` (mono, `--surface-2` on-fill) for the MNQ1!/MES1! symbol toggle. **Every mutually-exclusive pill row sits inside a `.cs-seg-track`** (user ruling 2026-07-11): a hairline `--r-pill` track wrapping the options so the row reads as ONE contained toggle; pills inside drop their own borders — the track is the only outline (session pickers, view tabs, DET/AI, FEED/POSITIONS, channel pickers, SegPills).
