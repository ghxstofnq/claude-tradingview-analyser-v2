...
- **Segmented pills.** `.cs-segpill` (transparent + 1px `--border` inactive → `--cs-onfill` + `--cs-onborder` active) for filters/toggles; `.cs-provpill` (borderless, same on-fill) for provider/view tabs; `.cs-sympill` (mono, `--surface-2` on-fill) for the MNQ1!/MES1! symbol toggle. **Every mutually-exclusive pill row sits inside a `.cs-seg-track`** (user ruling 2026-07-11, matched to the ⌘4 mode-switch reference): a `--surface-2` `--r-pill` track with a hairline border; options are `500 11.5px` sans at `4px 13px` with transparent borders; the active option carries `--cs-onfill` + `--cs-onborder` + weight 600 (session pickers, view tabs, DET/AI, FEED/POSITIONS, channel pickers, SegPills).
