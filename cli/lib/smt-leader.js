...
// selection for the MNQ/MES pair. Strategy authority: docs/strategy/daily-bias.md
// §6 (SMT / leading asset) — watch for ES↔NQ divergence (one makes a new
// extreme, the other fails); the leader signals direction, the weaker is the
// trade vehicle; decided in the NY open-reaction window. Selection only; never
// the entry trigger. Ported from PR #134 (claude/smt-leader-selection) with the
// opposite-sign-divergence + any-tier-pivot corrections; user-confirmed design.
//
// Hard rules:
...
// The window's price extreme on the requested side — daily-bias §6 "fails to confirm
...
export function computeSmtLeader({
  ...
  const evalSide = (side) => {
    ...
    // extreme (strength > 0) while the OTHER fails (strength <= 0) — daily-bias §6
