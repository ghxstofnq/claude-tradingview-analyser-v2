// Fresh-oracle Option A regression — 2026-06-18 MNQ.
//
// The tracked 06-18 tape is verified as the approved Trend reclaim-continuation
// long. The fresh capture-only MNQ/MES corpus is the newer source recording;
// folding the fresh MNQ tape through the same direct-brief reconstruction used
// by scripts/inspect-fresh-oracle-tapes.mjs must still reproduce that approved
// row if Option A is viable.
...
const TAPE = join(HERE, "tapes", "fresh-oracle", "2026-06-18-mnq-ny-am-replay.tape.json");
...
test("fresh 2026-06-18 MNQ folds to the approved Trend reclaim-continuation long", async () => {
  ...
  await runBacktest({
    ...
    bus,
    deps,
    stateDir: tmpStateDir("fresh-06-18-option-a-"),
  });

  const first = surfaced[0] ?? null;
  assert.ok(first, "expected fresh fold to surface the approved 06-18 Trend packet");
