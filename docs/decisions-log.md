...
## 2026-07-10 — Autonomous loop authorized: gates-green auto-merge + unrestricted chart access

User set up a /loop to drive the unified-goal program autonomously and granted two standing authorizations. (1) **Auto-merge when gates green:** loop-produced PRs self-merge once the verification matrix for the touched area is fully green — EXCEPT strategy levers, CLAUDE.md constraint changes, and anything that arms or routes real money, which always park for explicit user approval (this supersedes the absorbed plan's blanket merge-only-after-user-approval rule for the exempted-free cases). (2) **Chart/recording access anytime:** the app may be stopped and TV Desktop driven headless at any time — the user is not using those charts until the bot is ready ("I wont be using those charts until the bot is fully ready"). Never-place-trades and all user decision checkpoints remain untouched: the loop parks and notifies at every one.

## 2026-07-10 — Full pre-approval: the loop runs the goal to completion

User converted the remaining evidence-gated checkpoints into standing grants ("I want everything fully approved already"). (1) Corpus window auto-approves on green certification (approval record written citing this ruling; proven-unrecordable days become documented manifest exceptions). (2) Strategy levers replace per-session user review with a STRICT MECHANICAL FIDELITY GATE: auto-enable only if fold non-negative on BOTH symbols AND every hand-verified oracle/tape day still passes AND no moved session contradicts a transcript citation; failures stay default-off, reported not parked. (3) Runner ruling delegated: implement what transcripts clearly support, ambiguity keeps current behavior. (4) Paper/AUTO bar = 5 clean sessions (zero unresolved reconciliation/protection/parity events). (5) Pre-ruled constraint changes ship on their rulings. (6) The LIVE arm remains the single irreducible human action — Claude may not initiate real-money trading and the system requires the human type-"LIVE" arm; the loop ends with one notification when everything is green. Recorded in [docs/intent/2026-07-10-unified-goal.md](intent/2026-07-10-unified-goal.md) §Full pre-approval.

## 2026-07-10 (late) — GOAL SHIFT: user-directed build-and-live-test replaces the backtest-first gate

User ruling (verbatim intent): "I dont want to test if the bot works or not on backtesting first. Im going to tell you what the bot should do and I will test it live." This supersedes the evidence-first sequencing of docs/intent/2026-07-10-unified-goal.md §Full pre-approval: the corpus re-record → certification → parity → lever-fold → 5-clean-paper-session chain is NO LONGER THE GATE for iterating on bot behavior. New operating mode: the user specifies desired behavior → Claude builds it (unchanged engineering rigor: worktree PRs, tests, review, gates-green merge, deploy verification) → the user tests it on LIVE sessions and gives the next spec. The backtest/certification/fold machinery remains in the repo and available on demand, but runs only when the user asks. Unchanged and reaffirmed: Claude never places trades and never arms real money — the type-"LIVE" arm and per-session resume remain the user's deliberate manual actions; the always-on execution guardrails (valid stop, sizing caps, daily-loss halt, account gate, protection watchdog, boot reconciliation) stay active regardless of test mode. The 2026-07-10 rev-5 indicator (panel lean votes + current-state SMT) is the deployed baseline for live testing.
