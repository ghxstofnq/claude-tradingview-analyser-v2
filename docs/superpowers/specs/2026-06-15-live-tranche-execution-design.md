...
- `app/main/execution/config.js` + settings IPC — persist automation mode + max-adds + combined-cap **and the existing RISK GUARDRAILS (Max $/trade, Daily loss, Default $ risk)** into the gitignored exec config (defaults backtest-exact). Today the guardrails live only in renderer state and ride along on each manual ticket; in the auto modes there is no ticket, so the main-process tranche manager must read them from config to enforce the same gate (`checkOrder`) before firing. The settings popover writes to both (renderer state for the live ticket display + main config for auto-fire).
