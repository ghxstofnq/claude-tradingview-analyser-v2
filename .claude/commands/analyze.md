...
For the **confirmation candle** check, read `gates.pillar3.last_bar` (the single most-recent bar's facts) instead of just the 5-bar average from `gates.pillar2`. A clean confirmation candle in the strategy's sense has: `direction` matching your setup direction, `body_ratio >= 0.6`, and `close_position_in_range` on the favorable side (>0.7 for bullish, <0.3 for bearish). Use `gates.pillar3.last_bar_age_seconds` to sanity-check live-ness: if it's larger than the bar interval implied by `chart.resolution`, the data is stale (market closed or feed paused) and "confirmation" can't really be assessed.
