...
2. **Stale-engine short-circuit.** Before reading any of pillar3, check `gates.engine.meta.stale`. If `true`, the engine output is older than one bar — write "engine output is <emit_age_seconds>s stale" and skip the setup grade for this bar. Do NOT grade off potentially-frozen levels.
