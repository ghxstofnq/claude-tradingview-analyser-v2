# claude-tradingview-analyser — project rules for Codex

This file extends the user's global working agreement at `~/.Codex/AGENTS.md`. The global agreement still applies in full. This file documents project-specific decisions, constraints, and context.
...
- [docs/research/ai-consistency.md](docs/research/ai-consistency.md) — what produces consistent LLM behavior. Headline: "tool calling" is half-right; **grammar-constrained decoding against a schema** is the real mechanism. In a Codex session we approximate it via a tight slash-command schema, few-shot examples in `<example>` tags, self-check rules, and golden-set regression testing.
...
**Consult these before** designing a new analysis mode (tracker / scanner / backtester), changing `/analyze`, adding a new slash command that involves Codex reasoning over data, or modifying the hard constraints below. When proposing a behavioral change, cite the relevant research finding as authority.
...
5. **Screenshots are for verifications and tests only.** `./bin/tv screenshot` exists but its output never feeds analysis. Do not include screenshots in the `analyze` bundle. *Source: [docs/research/ai-trading-analysis.md](docs/research/ai-trading-analysis.md) — multimodal LLMs can answer correctly while barely using the image; screenshots risk visual hallucination.* **Exception (2026-07-10 ruling — docs/intent/2026-07-10-unified-goal.md §Full pre-approval):** the post-close journal assist (purpose `journal`) may read the auto-journal screenshot to draft the trade note the user edits/accepts. Screenshots remain banned from all analysis input; this exception is scoped to post-close journaling only.
...
10. **No backtesting on data Codex has seen.** When validating analyses on historical sessions, use post-cutoff dates or out-of-sample symbols. Frontier LLMs memorize prices and outcomes on widely-discussed pre-cutoff dates. *Source: [docs/research/ai-trading-analysis.md](docs/research/ai-trading-analysis.md).*
...
- Co-author tag on every commit: `Co-Authored-By: Codex <noreply@anthropic.com>`.
...
- Keep `AGENTS.md` as active operating guidance plus high-signal changelog only; move long completed-plan detail into dated docs when it starts to conflict with current behavior.
