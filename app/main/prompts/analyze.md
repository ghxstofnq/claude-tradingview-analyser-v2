...
- `gates.engine.meta` — `{schema, schema_supported, tf, emit_ny, symbol, emit_ms, emit_age_seconds, stale, engine_session}` provenance. **If `schema_supported` is false the engine bumped its format — say so and stop.** **If `stale: true` (emit_age_seconds > 90), the engine output is older than one bar — say "engine output is <N>s stale" and emit `surface_no_trade` rather than reading numbers that may be wrong.** `engine_session` is Pine's DST-aware classification (`asia|london|ny_am|ny_pm|off`) — if it disagrees with `gates.session.phase`, flag the drift.
- `gates.engine.price_context.{inside_fvgs, inside_bprs, nearest_opposing_fvg_above, nearest_opposing_fvg_below}` — engine zones containing current price plus the closest unfilled opposing FVG on either side. Each carries pre-computed `{distance_to_top, distance_to_bottom, distance_to_ce}` (signed; positive = price above). Cite these directly — do NOT subtract.
...
- `gates.engine.pillar1.{liquidity_pools, untaken_pools_above, untaken_pools_below}` — equal-high (`kind=eqh`) / equal-low (`kind=eql`) liquidity pools the engine maintains across the session (strategy §2.1 draw-target liquidity). `untaken_pools_above` are unswept EQH above current price sorted closest-first; `untaken_pools_below` are unswept EQL below sorted closest-first. Each pool `{kind, side, price, swept}`.
- `gates.engine.pillar2.{current_tf, m5, m15}` — each the engine quality verdict `{range_3h, range_quality (good|tight|na), displacement (clean|acceptable|weak|na), candle (engulfing|doji_wick|normal), atr_14, atr_17, session}`. `acceptable` displacement is workable but weaker than `clean`. ATRs are Wilder values the engine ships so the backend never recomputes them — cite the field directly when you need a magnitude.
- `gates.engine.pillar3.fvgs[]` — `{kind (fvg|ifvg), dir (bull|bear), top, bottom, ce, created_ms, took_liq, disp_score, reacted, reaction_dir, state (fresh|ce_tapped|filled|inverted|invalidated), size_quality (tiny|normal|large|unknown)}`. **`size_quality=tiny` zones are sub-tradable noise — skip them as setup FVGs.** `reacted=true` + `reaction_dir` says the zone already mitigated and in which direction. Pine keeps the most-recent 24 per TF (FIFO).
- `gates.engine.pillar3.fvgs_ranked[]` — same shape, pre-sorted by `(state=fresh DESC, took_liq DESC, disp_score DESC)`. **Use this when picking a setup FVG; `fvgs_ranked[0]` is the highest-priority candidate.** `fvgs[]` stays Pine order for raw inspection.
- `gates.engine.pillar3.{bprs[], swings:{internal[], swing[]}, structure_events[], structures_by_tier:{swing[], internal[]}, failure_swings[], most_recent_structure, fvg_summary}` — each swing `{kind, price, bar_ms, tier, swept, is_high}`; each `structure_events` entry `{event (bos|mss), dir, level, displacement, tier, validation (break|sweep), confirmed_ms}`. `structures_by_tier` mirrors Pine's per-tier separation (external `swing` vs `internal` 5-bar pivots) — prefer `structures_by_tier.swing[]` for Trend/MSS reads on external pivots. `failure_swings[]` is the pre-filtered pool of `event=mss + validation=sweep` — these are stop-run reversals (strongest reversal cue in the engine). `most_recent_structure` is the latest by `confirmed_ms`.
...
2. **Stale-engine short-circuit.** Before reading any of pillar3, check `gates.engine.meta.stale`. If `true`, the engine output is older than one bar — write "engine output is <emit_age_seconds>s stale" and emit `surface_no_trade` with reason `engine output stale`. Do NOT grade a setup off potentially-frozen levels.

3. **Walk all three entry models — explicitly, by name, every bar.** Read `gates.engine.pillar3.most_recent_structure`, `gates.engine.pillar3.structures_by_tier.swing` (external pivots, ignore internal for the MSS/Trend reads unless you're zoomed in), `gates.engine.pillar3.failure_swings` (pre-filtered stop-run MSS pool), `gates.engine.price_context.inside_fvgs`, `gates.engine.pillar3.fvgs_ranked` (use this in preference to raw `fvgs`), and `gates.engine.pillar3.fvg_summary`. Then state a one-line verdict for **each** of MSS / Trend / Inversion — do not stop at the first model that doesn't fit. Grade each model ONLY on its own components (`entry-models.md`); never disqualify one model with another model's rule.

   - **MSS** — a sweep of a swing low/high → sharp displacement back the other way → retrace into the freshly-created FVG → confirmation close. The engine emits this mechanically: a `structure_events` entry with `event=mss`, `displacement=true`, and a matching fresh `fvg`. **`gates.engine.pillar3.failure_swings[]` is the strongest MSS pool — `validation=sweep` means the break came after a stop-run, which is the textbook ICT failure-swing reversal.** Cite from `failure_swings[]` when the entry is one of these.
   - **Trend** — HTF + LTF aligned, an impulse leg, pullback into an internal FVG **with structure intact (higher highs / higher lows)** → confirmation close. A `structure_events` entry with `event=bos` (especially in `structures_by_tier.swing[]`) is the engine's continuation signal. The "structure intact" requirement is Trend-only.
   - **Inversion** — an opposing-direction FVG violated by a strong close → optional retest of the inverted zone → confirmation close. The engine emits this directly: an `fvg` with `kind=ifvg` and `state=inverted`. The `reacted` + `reaction_dir` fields tell you whether the retest already produced a directional close. Inversions form *during* a pullback — **a broken higher low does NOT disqualify an Inversion** (that is a Trend-model rule).

   Guards (each cost a real trade on 2026-05-20):
   - **FVG size** — use the engine's `size_quality` field: `tiny` zones are sub-tradable noise, skip them; `normal` and `large` are fine. Do not eyeball gap size — the engine has already classified it width-vs-ATR. `disp_score` tells you the displacement strength.
   - **The engine quality fields are a hint, not a veto** — `gates.engine.pillar2.m5.candle` / `displacement` are lagging summaries. Judge the displacement *at* the setup, and override when you disagree. (`displacement=acceptable` is workable; only `weak` should make you hesitate.)
...
4. **If a candidate is forming or has fired**, append to `<sdir>/setups.jsonl`:
